...
public:
	...
	inline u32 decodeValue(const u64 &pos)
	{
		...
		if (flagRank->checkOne(offsetBlock)) // gama coding
		{
	...
	inline u32 decodeGama(u64 &pos, const u32 &offset)
	{
		...
		while (1)
		{
			...
			else
			{
				...
				return this->bm->uncode(index, value); //sequence decoding the bitMap
			}
		}
		return -1; // it can must return a value in this method
	}
	inline void decodeLeafList(const register u64 &begin, const register u64 &end, BTuple bt[], int &count)
	{
		count = 0;
