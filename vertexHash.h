...
private:
	string hashTable[1001]; // store the feature
	int length[1001];
	map<int, int> offsetID; //recording the mapping relationship: offset->branchID
	int vertexIndex;
	string SENTRY;
...
public:
	...
	void sequenceHash(string hashFile)
	{
		...
		int size;
		size = offsetID.size();
		fwrite(&vertexIndex, sizeof(int), 1, fw);
	...
	void readHashTable(string hashFile)
	{
		...
		int size;
		char ch[16];
		fread(&vertexIndex, sizeof(int), 1 ,fr);
